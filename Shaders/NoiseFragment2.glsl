...
void main(void){
	float pi = 3.141592;
	vec2 screen = vec2(1280, 720);
	vec2 pos = gl_FragCoord.xy / screen;
	
	// Line from centre to point
	vec2 join = pos - vec2(0.5);
	// Angle of point from the vertical
	vec2 vertical = vec2(0.0, 1.0);
	float angle = acos(dot(normalize(join), vertical));
	// No acos which gives [0, 2pi] so have to adjust
	if (pos.x < 0.5) {
		angle = 2 * pi - angle;
	}
	//gl_FragColor = vec4(vec3(angle), 1.0);
	//return;
	float sweepPeriod = 2.0;
	float sweepRate = 2 * pi / sweepPeriod;
	float sweep = 0.0 + sweepRate * mod(time / 1000.0, sweepPeriod);
