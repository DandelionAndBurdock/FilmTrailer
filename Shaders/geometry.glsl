...
const float texStep = 1.0 / 8.0; // Each tex coordinate runs from 0.0 to 1.0 divide into 8 sprites
...
//number of vertices in the bound VBO
	...
	//using square brackets, just like C++ arrays
		...
		//struct...
		float row = IN[i].texIndices.x;
		float col = IN[i].texIndices.y;
		//top right
		gl_Position = gl_in[i].gl_Position;
		gl_Position.x += particleSize;
		gl_Position.y += particleSize;
		OUT.texCoord = vec2(texStep *(row + 1.0), texStep *(col + 1.0));
		...
		gl_Position.x -= particleSize;
		gl_Position.y += particleSize;
		OUT.texCoord = vec2(texStep *(row + 0.0), texStep *(col + 1.0));
		EmitVertex();
		...
		gl_Position.x += particleSize;
		gl_Position.y -= particleSize;
		OUT.texCoord = vec2(texStep *(row + 1.0), texStep *(col + 0.0));
		EmitVertex();
		...
		gl_Position.x -= particleSize;
		gl_Position.y -= particleSize;
		OUT.texCoord = vec2(texStep *(row + 0.0), texStep *(col + 0.0));
		EmitVertex();
