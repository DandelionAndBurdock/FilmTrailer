...
void	MD5Node::Update(float msec) {
	if(currentAnim) {
		...
		unsigned int prevAnimFrame = currentAnimFrame;
		while(frameTime < 0) {
		...
		if (currentAnimFrame < prevAnimFrame) {
			++numAnimCycles;
		}
...
bool	MD5Node::GetJointWorldTransform(const std::string&name, Matrix4 &t) {
...
bool	MD5Node::GetJointLocalTransform(const std::string&name, Matrix4 &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
...
bool	MD5Node::GetParentLocalOrientation(const std::string&name, Quaternion &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
...
bool	MD5Node::GetParentWorldOrientation(const std::string&name, Quaternion &t) {
	return false;
}


bool	MD5Node::GetParentLocalTransform(const std::string&name, Matrix4 &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
...
bool	MD5Node::GetParentWorldTransform(const std::string&name, Matrix4 &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
...
bool	MD5Node::SetJointLocalTransform(const std::string &name, Matrix4 &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
...
bool	MD5Node::SetJointWorldTransform(const std::string &name, Matrix4 &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
