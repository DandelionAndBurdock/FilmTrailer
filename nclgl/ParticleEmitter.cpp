...
Particle* ParticleEmitter::GetFreeParticle()	{
	...
	p->colour		= glm::vec4(1.0, 1.0, 1.0, 1.0);
	p->direction	= initialDirection;
	p->direction.x += ((RAND()-RAND()) * particleVariance);
	p->direction.y += 2.0;
	p->direction.z += ((RAND()-RAND()) * particleVariance);

	glm::normalize(p->direction);	//Keep its direction normalised!
	p->position = position;
	p->position.x += ((RAND() - RAND()) * 5.0f * particleVariance);
	p->position.z += ((RAND() - RAND()) * 5.0f *particleVariance);
...
void	ParticleEmitter::ResizeArrays() {
	...
	vertices = new glm::vec3[particles.size()];
	colours  = new glm::vec4[particles.size()];
	textureCoords = new glm::vec2[particles.size()];
	...
	glBufferData(GL_ARRAY_BUFFER, particles.size()*sizeof(glm::vec4), 0, GL_DYNAMIC_DRAW);

	glGenBuffers(1, &bufferObject[TEXTURE_BUFFER]);
	glBindBuffer(GL_ARRAY_BUFFER, bufferObject[TEXTURE_BUFFER]);
	glBufferData(GL_ARRAY_BUFFER, particles.size() * sizeof(glm::vec2), 0, GL_DYNAMIC_DRAW);
...
void ParticleEmitter::Draw()	{
	...
	for(unsigned int i = 0; i < particles.size(); ++i) {
		...
		float lifetime = particles.at(i)->colour.w;
		float lifePassed = particleLifetime - lifetime;

		//float timePerFrame = lifetime * 64.0f;// particleLifetime / 64.0f;
		float timePerFrame = particleLifetime / 64.0f;

		int frameNo = (1 - lifetime) * 64;//lifePassed / timePerFrame;

		int col = frameNo % 8;
		int row = (frameNo-col) / 8;
		//std::cout << col << " "  << row << std::endl;

		textureCoords[i] = glm::vec2(float(row), float(col));


		//vertices[i] = glm::vec3(col * 5.f, row *5.f, 0.f);;
		//colours[i] = glm::vec4(1.f, 1.f, 1.f, 1.f);
		//textureCoords[i] = glm::vec2(float(col), float(row));
#
	}
	...
	glBindBuffer(GL_ARRAY_BUFFER, bufferObject[VERTEX_BUFFER]);
	glBufferSubData(GL_ARRAY_BUFFER, 0, particles.size()*sizeof(glm::vec3), (void*)vertices);
	glVertexAttribPointer(VERTEX_BUFFER, 3, GL_FLOAT, GL_FALSE, sizeof(glm::vec3), 0);	//Tell the VAO we have positions...
	glEnableVertexAttribArray(VERTEX_BUFFER);	//Binds this buffer to the VAO
	
	//And now do the same for colours...
	glBindBuffer(GL_ARRAY_BUFFER, bufferObject[COLOUR_BUFFER]);
	glBufferSubData(GL_ARRAY_BUFFER, 0, particles.size()*sizeof(glm::vec4), (void*)colours);
	glVertexAttribPointer(COLOUR_BUFFER, 4, GL_FLOAT, GL_FALSE, sizeof(glm::vec4), 0);
	glEnableVertexAttribArray(COLOUR_BUFFER);

	// Texture Indices
	glBindBuffer(GL_ARRAY_BUFFER, bufferObject[TEXTURE_BUFFER]);
	glBufferSubData(GL_ARRAY_BUFFER, 0, particles.size() * sizeof(glm::vec2), (void*)textureCoords);
	glVertexAttribPointer(TEXTURE_BUFFER, 2, GL_FLOAT, GL_FALSE, sizeof(glm::vec2), 0);
	...
	SHADER_MANAGER->SetShader("ParticleShader");
	glActiveTexture(GL_TEXTURE0);
	TEXTURE_MANAGER->BindTexture("Explosion");
	SHADER_MANAGER->SetUniform("ParticleShader", "particleSize", particleSize);
