	...
	for(unsigned int i = 0; i < type.numSubMeshes; ++i) {
		...
		*/
		for(int j = 0; j < subMesh.numverts; ++j) {
			...
			target->textureCoords[j]   = glm::vec2(subMesh.verts[j].texCoords.x, subMesh.verts[j].texCoords.y);

			//And we should start off with a Vector of 0,0,0
			target->vertices[j] = glm::vec3(0.0f);
			...
			*/
			...
			for(int k = 0; k < subMesh.verts[j].weightElements; ++k) {
				...
				*/
				Vector3 temp = (joint.transform * weight.position) * weight.weightValue;
				target->vertices[j] += glm::vec3(temp.x, temp.y, temp.z);				
			}
		}
...
void MD5Mesh::RebufferData()	{
	...
	glBufferSubData(GL_ARRAY_BUFFER, 0, numVertices * sizeof(glm::vec3), (void*)vertices);

	if(textureCoords) {
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[TEXTURE_BUFFER]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, numVertices*sizeof(glm::vec2), (void*)textureCoords);
	}

	if (colours)	{
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[COLOUR_BUFFER]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, numVertices*sizeof(glm::vec4), (void*)colours);
	}

#ifdef MD5_USE_NORMALS
	if(normals) {
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[NORMAL_BUFFER]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, numVertices*sizeof(glm::vec3), (void*)normals);
	}
#endif
	...
	if(tangents) {
		...
		glBufferSubData(GL_ARRAY_BUFFER, 0, numVertices*sizeof(glm::vec3), (void*)tangents);
	}
#endif
